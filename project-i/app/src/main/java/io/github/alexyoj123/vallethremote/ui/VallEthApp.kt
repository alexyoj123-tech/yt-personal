...
@Composable
fun VallEthApp(vm: AppViewModel) {
    ...
    LaunchedEffect(pendingApk) {
        if (pendingApk != null) tab = Tab.ENVIAR
    }
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = {
                    Column {
                        Text("VallEthRemote", style = MaterialTheme.typography.titleMedium)
                        Text(
...
private fun Contenido(tab: Tab, vm: AppViewModel, padding: PaddingValues) {
    ...
    when (tab) {
        ...
        Tab.ENVIAR -> ApkScreen(vm, modifier)
